...
from models.gan.attention import SNConv2d, SNLinear, SNConvTranspose2d
...
class MST(nn.Module):
    def __init__(self, in_dim=31, out_dim=31, dim=31, stage=2, num_blocks=[2,4,4]):
        ...
        # Input projection
        self.embedding = SNConv2d(in_dim, self.dim, 3, 1, 1, bias=False)
        ...
        for i in range(stage):
            self.encoder_layers.append(nn.ModuleList([
                ...
                SNConv2d(dim_stage, dim_stage * 2, 4, 2, 1, bias=False),
            ]))
            dim_stage *= 2
        ...
        for i in range(stage):
            self.decoder_layers.append(nn.ModuleList([
                SNConvTranspose2d(dim_stage, dim_stage // 2, stride=2, kernel_size=2),
        ...
        # Output projection
        self.mapping = SNConv2d(self.dim, out_dim, 3, 1, 1, bias=False)
...
class SN_MST_Plus_Plus(nn.Module):
    def __init__(self, in_channels=3, out_channels=31, n_feat=31, stage=3):
        super(SN_MST_Plus_Plus, self).__init__()
        self.stage = stage
        ...
        self.conv_out = SNConv2d(n_feat, out_channels, kernel_size=3, padding=(3 - 1) // 2,bias=False)
...
if __name__ == '__main__':
    model = SN_MST_Plus_Plus().cuda()
    train_model = BaseModel(opt, model, model_name='SN_MST_Plus_Plus')
    train_model.train()
