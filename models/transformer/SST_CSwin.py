...
class CSWinB_CrossAttn(nn.Module):
    def __init__(self, dim, reso, num_heads,
        ...
        self.ffn1 = SGFN(dim, dim * 4, dim)
        self.ffn2 = SGFN(dim, dim * 4, dim)
    ...
    def forward(self, x, h, w):
        ...
        attened_x = torch.cat([x1,x2], dim=2)
        attened_x = self.proj1(attened_x)
        x = x + attened_x
        x = x + self.ffn1(self.norm2(x), h, w)

        x = self.norm3(x)
        x3 = self.crossattns[0](x[:,:,:C//2], x[:,:,C//2:])
        x4 = self.crossattns[1](x[:,:,C//2:], x[:,:,:C//2])
        attened_x = torch.cat([x3,x4], dim=2)
        attened_x = self.proj2(attened_x)
        x = x + attened_x
...
class Spatial_MSAB(nn.Module):
    def __init__(self, dim, head, resolution, split_size, patch_size = 8):
        super().__init__()
        ...
        # self.cat = LePE_CAT_Block(dim, [resolution, resolution], patch_size)

    def forward(self, x, h, w):
        if h != self.resolution or w != self.resolution:
            self.cswin.change_resolution(h)
            self.cat.change_resolution([h, w])
...
class LePE_CAT_Block(nn.Module):
    def __init__(self, dim, resolution, patch_size) -> None:
        ...
        self.resolution = resolution
        self.norm1 = nn.LayerNorm(dim)
        self.cat = LePE_CAT(dim, [resolution, resolution], patch_size)
        self.norm2 =nn.LayerNorm(dim)
        self.sgfn = SGFN(dim, dim * 4, dim)
...
class SSTransformer(nn.Module):
    def __init__(self, in_dim = 3, out_dim = 31, hidden_dim = 32, split_size = 1, input_resolution = [128, 128], n_blocks = [1, 2, 3], bottle_depth = 4, n_refine = 2, patch_size = 8):
        ...
        self.bottle_layer = SSTLayer(new_ch, self.head, self.input_resolution, self.split_size, bottle_depth)
