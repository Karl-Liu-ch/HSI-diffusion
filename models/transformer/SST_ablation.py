...
class Spectral_MSA(nn.Module):
    def __init__(self, dim, num_heads, bias, use_rpe = True):
...
class Spectral_MSAB(nn.Module):
    def __init__(self, dim, head, use_rpe = True):
        super().__init__()
        self.norm1 = nn.LayerNorm(dim)
        self.s_msa = Spectral_MSA(dim, dim // head, False, use_rpe=use_rpe)
...
class CATLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, patch_size, mlp_ratio=4., qkv_bias=True, 
                 ...
                 norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False, use_spatial = True, use_spectral = True, use_rpe = True):
        ...
        for i in range(depth):
            ...
            self.post_ipsa_blocks.append(CATBlock(dim=dim, input_resolution=input_resolution,
                                                  ...
                                                  attn_drop=ipsa_attn_drop, drop_path=drop_path,
                                                  norm_layer=norm_layer, attn_type="ipsa", rpe=True))
            self.spectral_blocks.append(Spectral_MSAB(dim, num_heads, use_rpe))
...
class SSTransformer(nn.Module):
    def __init__(self, in_dim = 3, out_dim = 31, hidden_dim = 32, split_size = 1, input_resolution = [128, 128], n_blocks = [1, 2, 3], bottle_depth = 4, n_refine = 2, patch_size = 8, use_spatial = True, use_spectral = True, use_rpe = True):
        ...
        self.bottle_layer = CATLayer(new_ch, self.input_resolution, bottle_depth, self.head, patch_size, use_spatial=use_spatial, use_spectral=use_spectral, use_rpe = use_rpe)
