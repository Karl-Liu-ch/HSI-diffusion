...
class CATLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, patch_size, out_dim = None, mlp_ratio=4., qkv_bias=True, 
    ...
    def forward(self, x):
        ...
        if self.downsample is not None:
            x = self.downsample(x)
        x = torch.concat([x, x_dense], dim=2)
        x = self.proj(x)
        x = rearrange(x, 'b (h w) c -> b c h w', h = h, w = w)
        return x
...
class SSTransformer_D(nn.Module):
    def __init__(self, in_dim = 3, out_dim = 31, expand_dim = 16, split_size = 1, n_blocks = 4, input_resolution = [128, 128], patch_size = 8, head = 4):
        super().__init__()
        self.head = head
        self.split_size = split_size
        self.input_resolution = input_resolution
        self.min_window = patch_size
        expand_dim = default(expand_dim, out_dim)
        self.expand_dim = expand_dim
        new_ch = expand_dim
        self.embed = nn.Conv2d(in_dim, expand_dim, 3, 1, 1)
        self.bottle_layer = nn.ModuleList([])
        for i in range(n_blocks):
            self.bottle_layer.append(CATLayer(new_ch, self.input_resolution, 1, self.head, patch_size, out_dim=new_ch + expand_dim))
            new_ch += expand_dim
            self.head = 4 * head
        self.to_out = nn.Conv2d(new_ch, expand_dim, 3, 1, 1)
        self.to_out_last = nn.Conv2d(expand_dim, out_dim, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        for layer in self.bottle_layer:
            x = layer(x)
        x = self.to_out(x) + x_residual
        out = self.to_out_last(x)

        return out[:, :, :h_inp, :w_inp]

if __name__ == '__main__':
    model = SSTransformer_D(6, 31)
