...
class SSTB(nn.Module):
    def __init__(self, dim, head, resolution, split_size):
        ...
        self.sst = CATLayer(dim, resolution, 1, head, patch_size = 8)
...
class LePE_CAT(nn.Module):
    def __init__(self, dim, resolution, patch_size, dim_out=None, num_heads=1, attn_drop=0., proj_drop=0., qk_scale=None, qkv_bias = False):
        super().__init__()
        self.dim = dim
    ...
    def forward(self, x):
        ...
        B_, N, _ = q.shape
        q = q.reshape(B_, N, self.num_heads, self.patch_size ** 2 // self.num_heads).permute(0, 2, 1, 3)
        k = k.reshape(B_, N, self.num_heads, self.patch_size ** 2 // self.num_heads).permute(0, 2, 1, 3)
        v = v.reshape(B_, N, self.num_heads, self.patch_size ** 2 // self.num_heads).permute(0, 2, 1, 3)
...
class LePE_CAT_Block(nn.Module):
    def __init__(self, dim, resolution, patch_size) -> None:
        super().__init__()
        self.resolution = resolution
        self.norm1 = nn.LayerNorm(dim)
        self.cat = LePE_CAT(dim, resolution, patch_size)
        self.norm2 =nn.LayerNorm(dim)
        self.sgfn = SGFN(dim, dim * 4, dim)
    ...
    def forward(self, x):
        h = self.resolution[0]
        w = self.resolution[1]
        x = x + self.cat(self.norm1(x))
        x = x + self.sgfn(self.norm2(x), h, w)
        return x
    
    def change_resolution(self, new_resolution):
        self.resolution = new_resolution
        self.cat.change_resolution(new_resolution)

class CATBlockConv(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, patch_size=8, mlp_ratio=4., 
        ...
        self.mlp = SGFN(dim, mlp_hidden_dim, dim)

    def patch(self, patches, C):
        patches = partition(patches, self.patch_size)  # nP*B, patch_size, patch_size, C
        patches = patches.view(-1, self.patch_size * self.patch_size, C)  # nP*B, patch_size*patch_size, C
        return patches
    ...
    def forward(self, x):
        ...
        attn = (torch.nn.functional.normalize(q, dim=-1) @ torch.nn.functional.normalize(k, dim=-1).transpose(-2, -1)) * self.scale
        attn = attn.softmax(dim=-1)
        attn = (attn @ v).transpose(1, 2).reshape(B, L, C)
...
class CATLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, patch_size, mlp_ratio=4., qkv_bias=True, 
        ...
        for i in range(depth):
            ...
            self.pre_ipsa_blocks.append(CATBlockConv(dim=dim, input_resolution=input_resolution,
                                                 num_heads=num_heads, patch_size=patch_size,
                                                 mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, 
                                                 qk_scale=qk_scale, drop=drop, 
                                                 attn_drop=ipsa_attn_drop, drop_path=drop_path,
            ...
            self.cpsa_blocks.append(LePE_CAT_Block(dim=dim, resolution=input_resolution, patch_size=patch_size))
