...
class TransBlock(nn.Module):
    def __init__(self, in_channels, spatial_size, wl_decomp, out_channels=None):
        ...
        self.conv1x1 = nn.Conv2d(in_channels=2*in_channels, out_channels=in_channels, kernel_size=1)
        self.ln5_train = nn.LayerNorm([in_channels, spatial_size[0], spatial_size[1]])
        self.ln5_test = nn.LayerNorm([in_channels, spatial_size_test[0], spatial_size_test[1]])
    
    def forward(self, input):
        b, c, h, w = input.shape
        if self.spatial_size != (h,w):
            self.train_size = False
        # if self.train_size: spabranch = self.ln2_train(torch.add(self.spa_att(self.ln1_train(input)), input))
        # if not self.train_size: spabranch = self.ln2_test(torch.add(self.spa_att(self.ln1_test(input)), input))
        mid = F.layer_norm(input, [self.in_channels, h, w])
        spabranch = F.layer_norm(torch.add(self.spa_att(mid), input), [self.in_channels, h, w])

        spabranch = torch.add(self.spa_ffn(spabranch), spabranch)

        # if self.train_size: specbranch = self.ln3_train(input)
        # if not self.train_size: specbranch = self.ln3_test(input)
        specbranch = F.layer_norm(input, [self.in_channels, h, w])

        specatt, attention_norm = self.spec_att(specbranch)
        specbranch = torch.add(specatt, specbranch)
        
        # if self.train_size: specbranch = torch.add(self.spec_ffn(self.ln4_train(specbranch)), specbranch)
        # if not self.train_size: specbranch = torch.add(self.spec_ffn(self.ln4_test(specbranch)), specbranch)
        mid = F.layer_norm(specbranch, [self.in_channels, h, w])
        specbranch = torch.add(self.spec_ffn(mid), specbranch)
        
        output = torch.cat((spabranch, specbranch), dim=1)
        output = self.conv1x1(output)
        
        # if self.train_size: output = self.ln5_train(output)
        # if not self.train_size: output = self.ln5_test(output)
...
class UNet(nn.Module):
    def __init__(self):
        ...
        self.embedding = nn.Conv2d(in_channels=3, out_channels=64, kernel_size=3, padding=1)
        self.trans_block_1 = TransBlock(in_channels=64, spatial_size=(128, 128), wl_decomp=1)

        self.down1 = PixelUnshuffleBlock(in_channels=64, out_channels=128)
        self.trans_block_2 = TransBlock(in_channels=128, spatial_size=(64,64), wl_decomp=2)
        
        self.down2 = PixelUnshuffleBlock(in_channels=128, out_channels=256)
        
        self.bottleneck_1 = TransBlock(in_channels=256, spatial_size=(32,32), wl_decomp=3)
        self.bottleneck_2 = TransBlock(in_channels=256, spatial_size=(32,32), wl_decomp=3)

        self.up1 = PixelShuffleBlock(in_channels=256, out_channels=128)

        self.trans_block_3 = TransBlock(in_channels=128, spatial_size=(64,64), wl_decomp=2)

        self.up2 = PixelShuffleBlock(in_channels=128, out_channels=64)

        self.trans_block_4_1 = TransBlock(in_channels=64, spatial_size=(128, 128), wl_decomp=1)
        self.trans_block_4_2 = TransBlock(in_channels=64, spatial_size=(128, 128), wl_decomp=1)
        
        self.conv_rescon1 = nn.Conv2d(in_channels=128, out_channels=64, kernel_size=1)
        self.conv_rescon2 = nn.Conv2d(in_channels=256, out_channels=128, kernel_size=1)
    ...
    def forward(self, input):
        ...
        rescon1, attnorm_1 = self.trans_block_1(input)
        output = self.down1(rescon1)
        rescon2, attnorm_2 = self.trans_block_2(output)
        output = self.down2(rescon2)
        output, attnorm_3 = self.bottleneck_1(output)
        output, attnorm_4 = self.bottleneck_2(output)
        output = self.up1(output)
        rescon2 = torch.cat((rescon2, output), dim=1)
        rescon2 = self.conv_rescon2(rescon2)
        output = torch.add(output, rescon2)
        output, attnorm_5 = self.trans_block_3(output)
        output = self.up2(output)
        rescon1 = torch.cat((rescon1, output), dim=1)
        rescon1 = self.conv_rescon1(rescon1)
        output = torch.add(output, rescon1)
        output, attnorm_6 = self.trans_block_4_1(output)
        output, attnorm_7 = self.trans_block_4_2(output)
        output = torch.add(output, input)
        output = self.out(output)
        attnorm = (attnorm_1 + attnorm_2 + attnorm_3 + attnorm_4 + attnorm_5 + attnorm_6 + attnorm_7) / 7
        return output[:, :, :h_inp, :w_inp]
        # return output, attnorm

if __name__ == '__main__':
    model = UNet()
    input = torch.rand(1,3,482,512)
    output = model(input)
    print(output.shape)
