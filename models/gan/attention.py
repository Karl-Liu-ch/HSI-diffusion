...
class Conv_MS_MSA(nn.Module):
    def __init__(
        ...
        self.num_heads = heads
        self.dim_head = dim_head
        self.to_q = SNConv2d(dim, dim_head * heads, 3, 1, 1, bias=False, groups=dim)
        self.to_k = SNConv2d(dim, dim_head * heads, 3, 1, 1, bias=False, groups=dim)
        self.to_v = SNConv2d(dim, dim_head * heads, 3, 1, 1, bias=False, groups=dim)
        self.rescale = nn.Parameter(torch.ones(heads, 1, 1))
