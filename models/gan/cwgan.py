...
class Generator(nn.Module):
    def __init__(self, num_input, num_output):
        ...
        def Conv(input_nums, output_nums):
            layer = []
            layer.append(nn.Conv2d(input_nums, output_nums, kernel_size=(3,3), stride=(1,1), padding=(1,1), bias=False))
        ...
        self.Net = nn.Sequential(
            ...
            *Conv(64, 256),
            *Conv(256, 256),
            nn.Conv2d(256, num_output, kernel_size=(3,3), stride=(1,1), padding=(1,1), bias=False),
            # nn.Tanh()
        )
...
class CWGAN():
    def __init__(self, opt, multiGPU = False) -> None:
        ...
        self.opt = opt
        self.G = Generator(6, 31)
        self.D = Discriminator(34)
        if self.multiGPU:
...
if __name__ == '__main__':
    ...
    spec = CWGAN(opt, multiGPU=opt.multigpu)
