...
class ConditionalVAESPCA(nn.Module):
    ...
    def __init__(self, in_channels: int, condition_channels: int,) -> None:
        ...
        self.fc_mu0 = MSAB(dim = in_channels, dim_head=in_channels, heads=1, num_blocks=2)
        self.fc_var0 = MSAB(dim = in_channels, dim_head=in_channels, heads=1, num_blocks=2)
        self.fc_mu1 = MSAB(dim = hidden_dims[0], dim_head=hidden_dims[0] // 4, heads = 4, num_blocks=4)
        self.fc_var1 = MSAB(dim = hidden_dims[0], dim_head=hidden_dims[0] // 4, heads = 4, num_blocks=4)
        self.fc_mu2 = MSAB(dim = hidden_dims[1], dim_head=hidden_dims[1] // 8, heads = 8, num_blocks=4)
        self.fc_var2 = MSAB(dim = hidden_dims[1], dim_head=hidden_dims[1] // 8, heads = 8, num_blocks=4)

        # Build Decoder        
        self.decoder_input0 = MSAB(dim = hidden_dims[1] * 2, dim_head=hidden_dims[1] // 8, heads=16, num_blocks=4)
        self.decoder_input1 = MSAB(dim = hidden_dims[0] * 2, dim_head=hidden_dims[0] // 4, heads=8, num_blocks=4)
        self.decoder_input2 = MSAB(dim = in_channels + condition_channels, dim_head=in_channels + condition_channels, heads=1, num_blocks=2)
