...
class FirstStageDecoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
        ...
        self.mid.conv_2 = nn.Conv2d(in_channels=block_in*2, out_channels=block_in*2, kernel_size=1, stride=1)
        self.mid.block_2 = ResnetBlock(in_channels=block_in*2,
                                       out_channels=block_in,
                                       temb_channels=self.temb_ch,
                                       dropout=dropout)
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            for i_block in range(self.num_res_blocks+1):
                if i_block == 0:
                    block.append(ResnetBlock(in_channels=block_in+block_out,
                                            out_channels=block_out,
                                            temb_channels=self.temb_ch,
                                            dropout=dropout))
                else:
    ...
    def forward(self, z, cond_features):
        ...
        h = self.mid.block_1(h, temb)
        h = self.mid.attn_1(h)
        h = self.mid.conv_2(torch.concat([h, cond_features[-2]], dim=1))
        h = self.mid.block_2(h, temb)
